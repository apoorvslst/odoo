import React, { useState } from 'react';
import { ArrowLeft, Check, BookOpen } from 'lucide-react';

/**
 * JournalForm Component
 * 
 * Manages Accounting Journals (e.g. Customer Invoices, Vendor Bills, Bank Operations, Cash).
 * Links each Journal to a default Chart of Accounts entry.
 */
export default function JournalForm({ journalId, journals = [], chartOfAccounts = [], onSave, onBack }) {
  const existing = journals.find(j => j.id === journalId);

  // STATE MANAGEMENT:
  // Tracks journal ID, Name, Type (Sales, Purchase, Bank, Cash), and linked Default Account
  const [formData, setFormData] = useState(existing || {
    id: `J0${journals.length + 1}`,
    name: '',
    type: 'Sales', // Options: 'Sales', 'Purchase', 'Bank', 'Cash'
    defaultAccount: chartOfAccounts[0]?.id || ''
  });

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.name.trim()) return;

    onSave(formData);
  };

  return (
    <div className="bg-white rounded-2xl shadow-[0_15px_35px_rgba(50,50,93,0.06)] border border-slate-200 overflow-hidden">
      {/* Top Action Bar */}
      <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-white">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition cursor-pointer"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Journals
        </button>

        <button
          type="button"
          onClick={handleSubmit}
          className="inline-flex items-center gap-2 px-5 py-2 text-sm font-semibold text-white bg-[#635BFF] hover:bg-[#5851DF] rounded-xl shadow-[0_4px_12px_rgba(99,91,255,0.35)] transition cursor-pointer"
        >
          <Check className="w-4 h-4" /> {existing ? 'Update Journal' : 'Save Journal'}
        </button>
      </div>

      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
          <div className="w-12 h-12 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center">
            <BookOpen className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-[#0A2540]">
              {existing ? `Edit Journal: ${existing.name}` : 'New Accounting Journal'}
            </h2>
            <p className="text-sm text-slate-500">
              Set up dedicated journals for sales, purchases, bank transactions, and cash ledgers.
            </p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Journal Name */}
          <div className="flex flex-col">
            <label className="mb-2 text-xs font-bold uppercase tracking-wider text-[#0A2540]">
              Journal Name <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              name="name"
              value={formData.name}
              onChange={handleChange}
              placeholder="e.g. Customer Invoices, Vendor Bills, Bank Operations"
              required
              className="px-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:ring-2 focus:ring-[#635BFF]/30 focus:border-[#635BFF] outline-none transition"
            />
          </div>

          {/* Journal Type (Dropdown: Sales, Purchase, Bank, Cash) */}
          <div className="flex flex-col">
            <label className="mb-2 text-xs font-bold uppercase tracking-wider text-[#0A2540]">
              Journal Type <span className="text-red-500">*</span>
            </label>
            <select
              name="type"
              value={formData.type}
              onChange={handleChange}
              required
              className="px-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:ring-2 focus:ring-[#635BFF]/30 focus:border-[#635BFF] outline-none transition"
            >
              <option value="Sales">Sales</option>
              <option value="Purchase">Purchase</option>
              <option value="Bank">Bank</option>
              <option value="Cash">Cash</option>
            </select>
          </div>

          {/* Default Account (Dropdown linked to CoA) */}
          <div className="flex flex-col">
            <label className="mb-2 text-xs font-bold uppercase tracking-wider text-[#0A2540]">
              Default Account (CoA) <span className="text-red-500">*</span>
            </label>
            <select
              name="defaultAccount"
              value={formData.defaultAccount}
              onChange={handleChange}
              required
              className="px-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:ring-2 focus:ring-[#635BFF]/30 focus:border-[#635BFF] outline-none transition"
            >
              <option value="">Select Default Account...</option>
              {chartOfAccounts.map((account) => (
                <option key={account.id} value={account.id}>
                  {account.id} - {account.name} ({account.type})
                </option>
              ))}
            </select>
          </div>
        </div>
      </form>
    </div>
  );
}
