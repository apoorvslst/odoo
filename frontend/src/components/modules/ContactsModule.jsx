...
import { Button, Pagination, usePagedSearch, ModuleShell, StatusPill } from './ui';
...
const ContactListView = ({ contacts, contactInvoicesMap = {}, contactPaymentsMap = {}, user, onNew, onSwitchToKanban, onSelectRow, onDelete, onArchive, onApprove, onReject }) => {
      ...
      <div className="cm-table-container">
        <table className="cm-table">
          ...
          <tbody>
                    ...
                    <td className="cm-text-center" onClick={(e) => e.stopPropagation()}>
                      <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', flexWrap: 'wrap' }}>
                        {contact.status === 'pending_approval' && (
                          <>
                            <button type="button" onClick={() => onApprove(contact)} className="btn btn-primary btn-sm" title="Approve contact">Approve</button>
                            <button type="button" onClick={() => onReject(contact)} className="btn btn-danger btn-sm" title="Reject contact">Reject</button>
                          </>
                        )}
                        <button
                          type="button"
            ...
            {pageItems.length === 0 && (
              <tr>
                <td colSpan={isAdmin ? 10 : 9} className="cm-text-center" style={{ padding: '40px', color: '#94a3b8', fontWeight: 500 }}>
...
export default function ContactsModule({ user }) {
  ...
  const handleApproveContact = async (contact) => {
    try {
      await apiFetch(`/contacts/${contact.id}/approve`, { method: 'PATCH' });
      await loadContacts();
    } catch (e) { window.alert(e.message); }
  };

  const handleRejectContact = async (contact) => {
    if (!window.confirm(`Reject contact "${contact.name}"?`)) return;
    try {
      await apiFetch(`/contacts/${contact.id}/reject`, { method: 'PATCH' });
      await loadContacts();
    } catch (e) { window.alert(e.message); }
  };
