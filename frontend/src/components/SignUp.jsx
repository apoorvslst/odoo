...
const SignUp = ({ onNavigateToLogin, onNavigateToForgotPassword }) => {
    ...
    const validateForm = () => {
        const errors = {};
        if (formData.name.trim().length < 3) errors.name = 'Business or contact name must be at least 3 characters.';
    ...
    const parseApiError = async (response) => {
        ...
        if (response.status >= 500) return 'Registration service is temporarily unavailable. Try again later.';
        try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert('Registration submitted successfully. Your account is pending accountant approval.');
    ...
    const fields = [
        { id: 'signup-name',       label: 'Business / Contact Name', key: 'name',       type: 'text',     maxLength: 120, auto: 'organization',   placeholder: 'Azure Furniture' },
        { id: 'signup-email',      label: 'Email',                   key: 'email',      type: 'email',    maxLength: 254, auto: 'email',          placeholder: 'you@company.com' },
        { id: 'signup-mobile',     label: 'Mobile',                  key: 'mobile',     type: 'tel',      maxLength: 20,  auto: 'tel',             placeholder: '+91 9090090909' },
            ...
            <div className="fade-in" style={{ width: '100%', maxWidth: 380 }}>
                <section className="auth-card">
                    <header className="t-center" style={{ marginBottom: '1.5rem' }}>
                        ...
                        <p className="auth-badge">CUSTOMER / VENDOR REGISTRATION</p>
                    </header>

                    <form onSubmit={handleSubmit} className="stack" noValidate>
                        <div className="field">
                            <label htmlFor="signup-type" className="auth-label">Account Type</label>
                            <select
                                id="signup-type"
                                name="type"
                                value={formData.type}
                    ...
                    <p className="demo-note">
                        Your portal access activates only after an accountant approves your registration.
                    </p>
